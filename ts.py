...
To fix the issue where long territory names with commas or hidden line breaks are breaking your site, we need to clean the data in the Python script and update the JavaScript in the HTML template to handle those filenames correctly.

Here is the complete, corrected script.

Key Changes Made:
Data Scrubbing: parse_overpass now physically removes commas and newlines from territory names.

CSV Safety: Added quoting=csv.QUOTE_MINIMAL to the CSV writers. This ensures that if a name has a weird character, it gets wrapped in "quotes" so the columns don't shift.

Filenaming: safe_name now preserves your special characters (like ꢡ) but strictly removes slashes and commas.

JavaScript Fix: Updated the toggleTerritory function in the HTML to use the same logic for finding files (removing commas and slashes).
...
async function toggleTerritory(name, rel, state) {
    if (state) {
        if (!loadedData[name]) {
            // FIX: Match the Python safe_name logic
            const safeName = name.replace(/ /g, "_").replace(/\//g, "-").replace(/,/g, "");
            const file = `tdata/territory/${safeName}_${rel}.csv`;
            try {
                const rows = await loadCSV(file);
                loadedData[name] = {
