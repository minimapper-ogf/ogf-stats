...
def main():
    ...
    # Create HTML immediately on start
    with open(HTML_OUTPUT_PATH, "w", encoding="utf-8") as f:
        f.write(HTML_TEMPLATE)
    print(f"✓ HTML file created at {HTML_OUTPUT_PATH}. Open it now to watch updates.")
    ...
    for i, t in enumerate(territories):
        rel_id = t["rel"]
        try:
            print(f"[{i+1}/{len(territories)}] Requesting rel {rel_id}...")
        ...
        with open(hist_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if write_h: writer.writerow(["timestamp", "nodes", "ways", "relations", "areas", "total"])
    ...
    print(f"✓ Done. Final results in {LATEST_FILE}")
